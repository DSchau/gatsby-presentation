...
export const LetsGetReal = () => (
  <div>
    ...
    <Text textColor="secondary" fit>
      (just for a bit, this is a safe place)
    </Text>
  </div>
);
...
export const NewHotness = () => (
  <Link href="https://midwestjs.netlify.com" target="_blank" rel="noopener">
    <Image
      src={require('./images/new-midwestjs-optimized.jpg')}
      ...
      <GoToAction
        render={goTo => (
          <Fill style={{ margin: '1rem' }}>
            <Button onClick={() => goTo('well')}>🙌</Button>
          </Fill>
        )}
      />
      <GoToAction
        render={goTo => (
          <Fill style={{ margin: '1rem' }}>
            <Button onClick={() => goTo('poorly')}>💩</Button>
          </Fill>
        )}
      />
    </Layout>
...
export const Well = () => (
  <div>
    ...
    <GoToAction
      render={goTo => (
        <Fill style={{ margin: '1rem' }}>
          <Button onClick={() => goTo('performance-matters')}>➡️</Button>
        </Fill>
      )}
    />
  </div>
